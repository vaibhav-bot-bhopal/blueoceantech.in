    ...
    while ($row = mysqli_fetch_assoc($slots)) {
        if ($row['slot_state'] == $row['free_slot']) {
            ...
            if (mysqli_num_rows($usrdetails) > 0) {
                ...
                $sql_fetch = "SELECT * FROM participants as p LEFT JOIN slots ON p.slot = slots.id";
                ...
                if (mysqli_query($conn, $sql_update)) {
                    $sql_mail = mysqli_query($conn, "SELECT *FROM participants as p LEFT JOIN slots ON p.slot = slots.id LEFT JOIN cities ON p.city = cities.id  LEFT JOIN places ON p.place = places.id WHERE name = '" . $_POST['name'] . "' AND email = '" . $_POST['email'] . "' AND slot ='" . $_POST['slot'] . "' ORDER BY email DESC LIMIT 1");
